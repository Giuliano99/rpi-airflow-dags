...
def add_new_players():
    ...
    # ✅ Insert only the selected players from dart_matches_gold
    insert_new_players_query = """
        INSERT INTO elo_rankings (player, elo)
        SELECT DISTINCT player1, 1500 FROM dart_matches_gold
        WHERE player1 NOT IN (SELECT player FROM elo_rankings)
        UNION
        SELECT DISTINCT player2, 1500 FROM dart_matches_gold
...
def calculate_elo():
    ...
    CREATE TABLE IF NOT EXISTS elo_match_log (
        match_id INT PRIMARY KEY REFERENCES dart_matches_gold(match_id) ON DELETE CASCADE,
        player1 VARCHAR(100),
        player2 VARCHAR(100),
